...
1/ E1696 | cannot open source file "d3dx10.h"
Solution:
- Open solution explorer, right click project (the project is bolded), choose properties (at the end), linker -> input -> additional dependency -> edit -> write d3d10.lib.
- Install this:
...
2/ LNK2019 | unresolved external symbol main referenced in function "int __cdecl invoke_main(void)" (?invoke_main@@YAHXZ)
   LNK1120 | 1 unresolved externals
Solution:
Open the previous linker (right click project -> properties -> linker), choose all options, find SubSystem. It is currently Console. Change it to Windows.


Expected: a brick moves from the middle left to the right of the screen.
Exercise: make the brick moves from the top center to the bottom of the screen.
...
#define BRICK_START_X 316.0f
// #define BRICK_START_Y 236.0f
#define BRICK_START_Y 8.0f

#define BRICK_START_VX 0.2f
#define BRICK_START_VY 0.6f
...
void Update(DWORD dt)
{
    ...
    // brick_x, brick_y is brick's position
    // brick_vx, brick_vy is brick's speed
    // brick_x += brick_vx * dt;
    brick_y += brick_vy * dt;
    ...
    if (brick_y < 0)
    {
        brick_vy = -2.0f * brick_vy;
        brick_y = 0;
    }
    else if (brick_y > bottom_edge)
    {
        brick_vy = -0.5f * brick_vy;
        brick_y = bottom_edge;
    }
}
...
void Render()
{
    if (pD3DDevice != NULL)
    {
        ...
        // Print content to the window's title
        DebugOutTitle((wchar_t*)L"%s (%0.1f,%0.1f) vx:%0.1f vy:%0.1f", WINDOW_TITLE, brick_x, brick_y, brick_vx, brick_vy);
